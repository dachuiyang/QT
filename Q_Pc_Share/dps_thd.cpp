...
SOCK_ERR:
    if (INVALID_SOCKET != this->m_p_cs->m_socket)
    {
        closesocket(this->m_p_cs->m_socket);
    }
...
int Dps_Thd::ifs_heart()
{
    ...
    Net_Tools::send_buf(this->m_p_cs->m_socket,
                        (char*)&data_head,
                        sizeof(Data_Head));
}
...
int Dps_Thd::ifs_online(char *p_data)
{
    ...
    p_l->s_host_sock = this->m_p_cs->m_socket;
